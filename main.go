...
func (r Running) Calories() float64 {
	return ((float64(CaloriesMeanSpeedMultiplier) * r.meanSpeed() * CaloriesMeanSpeedShift) * r.Weight /
	float64(MInKm) * r.Duration.Hours() * float64(MinInHours))
}
...
func (w Walking) Calories() float64 {
	return ((CaloriesWeightMultiplier*w.Weight +
		(math.Pow(w.meanSpeed()*KmHInMsec, 2)/(w.Height/100))*
		CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * float64(MinInHours))
}
...
func (s Swimming) meanSpeed() float64 {
	return float64(s.LengthPool) * float64(s.CountPool) / float64(MinInHours) / s.Duration.Hours()
}
